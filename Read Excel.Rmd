...
```{r echo=TRUE}
cx <- list.files("data", ".xlsx", full.names = T) %>%
  xlsx_cells()
cx.colors <- cx %>%
...
## inspect formating differences among merged cells
...
ftb[1:nrow(ftb) %in% c(12, 13, 14, 25), ] %>% 
  ...
  data.frame()
```
Nothing indicate that one formate dictate the cell is a merged area.
